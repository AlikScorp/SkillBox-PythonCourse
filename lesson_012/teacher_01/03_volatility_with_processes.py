...
class Runner(Process):
    ...
    def run(self):
        ...
        if min_val and max_val:
            ...
            self.queue.put([round(volatility, 2), secid])
...
def main():
    ...
    for filename in files(SOURCE_PATH):
        runner = Runner(filename=filename, queue=store)
        runner.start()
    ...
    while need_continue:
        try:
            data = store.get(timeout=1)
            tickers.append(data)
        except queue.Empty:
            print(f'Queue empty')
            time.sleep(.001)
            need_continue = any([w.is_alive() for w in workers if w])
...
# Функция работала 1.7597 секунд(ы)
