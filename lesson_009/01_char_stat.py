...
class CharCounter:
    """
        ...
        # Можно выводить отсортировано чрез метод display с помощью параметра sorting (по умолчанию sorting = 0)
        # 0 - по алфавиту,
        # 1 - по алвавиту (реверсивно),
        # 2 - по количеству,
        # 3 - по количеству (реверсивно)
    """
    ...
    def display(self, sorting=0):
        ...
        if sorting == self.SORTED_BY_ALPHABET:
            keys.sort()  # Сортируем лист по возрастанию
            for key in keys:
                print(f"|{key:^10}|{self.symbols[key]:>10}|")
        elif sorting == self.SORTED_BY_ALPHABET_REVERSE:
            keys.sort(reverse=True)  # Сортируем лист по убыванию
            for key in keys:
                print(f"|{key:^10}|{self.symbols[key]:>10}|")
        elif sorting == self.SORTED_BY_QUANTITY:
            items.sort(key=operator.itemgetter(1))  # Сортируем лист (по возрастанию) по второму элементу кортежа
            for key, value in items:
                print(f"|{key:^10}|{value:>10}|")
        elif sorting == self.SORTED_BY_QUANTITY_REVERSE:
            items.sort(key=operator.itemgetter(1), reverse=True)  # Сортируем лист (по убыванию)
            for key, value in items:
                print(f"|{key:^10}|{value:>10}|")
        else:
            pass
...
if __name__ == '__main__':
    counter = CharCounter('voyna-i-mir.txt.zip')
    if counter.count():
        counter.display()
    else:
